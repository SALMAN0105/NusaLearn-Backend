            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto no-scrollbar p-6 lg:p-10 bg-p-xlt dark:bg-[#1e1b4b] dot-grid-bg transition-colors duration-300">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-5 bg-white dark:bg-[#2d2460] p-6 rounded-2xl shadow-neo border-2 border-black dark:border-p-dark fade-in fade-in-1">
                    ...
                    <form action="" method="GET" class="w-full md:w-auto flex-1 max-w-md relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Query nama / username..." 
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-p-dark/20 transition-colors group">
                                    <td class="px-6 py-5">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="font-extrabold text-black dark:text-white text-base">{{ $student->name ?? 'Unknown Entity' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-5 max-w-[200px]">
                                        <div class="text-black dark:text-white font-bold truncate flex items-center gap-2 text-sm" title="{{ $student->school_origin }}">
                                            <i class="fa-solid fa-school text-p-mid text-xs"></i> {{ $student->school_origin ?? 'Belum Terkalibrasi' }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-5 text-center">
                                    ...
                                    <td class="px-6 py-5 text-right">
                                        <form action="{{ route('students.destroy', $student->id ?? 0) }}" method="POST" class="inline-block" onsubmit="return confirm('Peringatan: Mencabut entitas siswa akan menghapus seluruh relasi progres pembelajaran. Lanjutkan?');">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="w-8 h-8 rounded-lg flex items-center justify-center bg-white border-2 border-black text-black hover:bg-neo-red transition-all shadow-neo-sm active:translate-x-[1px] active:translate-y-[1px] active:shadow-none" title="Drop User">
