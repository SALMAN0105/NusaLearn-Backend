...
<body class="bg-p-xlt text-black font-sans selection:bg-p-lt selection:text-p-dark dark:bg-[#1e1b4b] dark:text-gray-100 transition-colors duration-300 relative">
    ...
    <div id="confirm-modal" role="dialog" aria-modal="true">
        <div class="cm-card">
            ...
            <div class="cm-nama_file" id="cm-nama_file">â€”</div>
    ...
    <div id="notif-toast">
        ...
        <div class="nt-body">
            ...
            <div class="nt-sub"  id="nt-sub">â€”</div>
        </div>
    </div>
                                    ...
                                    <tr class="log-row hover:bg-gray-50 dark:hover:bg-p-dark/20 transition-colors group" data-id="{{ $history->id }}">
                                        <td class="px-5 py-4">
                                            ...
                                            <div class="text-[10px] font-bold text-gray-500 dark:text-purple-300/60 mt-1 uppercase tracking-wider flex items-center gap-1.5">
                                                ...
                                                @endif
                                                {{ str_replace('_', ' â†’ ', $history->tipe_konversi) }}
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-4 py-4 text-center">
                                            @if($history->status == 'success' && $history->jalur_output_json && $history->jalur_output_json !== '-')
                                                <a href="{{ route('administrator.converter.download', ['path' => base64_encode($history->jalur_output_json)]) }}"
    ...
    <script>
        ...
        document.getElementById('uploadForm').addEventListener('submit', async function(e) {
            ...
            if (isAI) {
                const pStart = parseInt(document.getElementById('page_start').value);
                const pEnd = parseInt(document.getElementById('page_end').value);
                if (isNaN(pStart) || isNaN(pEnd) || pStart < 1 || pEnd < pStart) {
                    alertBox.className = "mb-6 flex items-center gap-3 px-5 py-3 rounded-xl border-2 border-black shadow-neo-sm text-sm font-black bg-neo-red text-black uppercase tracking-wide fade-in";
                    alertBox.innerHTML = `<i class="fa-solid fa-triangle-exclamation text-xl"></i> <div>Harap masukkan rentang Hal. Awal dan Hal. Akhir yang valid (minimal 1).</div>`;
                    alertBox.classList.remove('hidden');
                    return;
                }
                if (pEnd - pStart > 19) {
                    alertBox.className = "mb-6 flex items-center gap-3 px-5 py-3 rounded-xl border-2 border-black shadow-neo-sm text-sm font-black bg-neo-red text-black uppercase tracking-wide fade-in";
                    alertBox.innerHTML = `<i class="fa-solid fa-triangle-exclamation text-xl"></i> <div>Batas aman memori: Maksimal 20 halaman per sekali tarik.</div>`;
                    alertBox.classList.remove('hidden');
                    return;
                }
            }
            ...
            try {
                const response = await fetch("{{ route('administrator.converter.process') }}", {
                    method: 'POST',
        ...
        function renderPagination() {
            ...
            document.getElementById('pagination-info').textContent = total > 0
                ? `Menampilkan ${start}â€“${end} dari ${total} log`
                : 'Tidak ada log';
        ...
        // ========================
        // HAPUS LOG (AJAX DELETE) â€” Custom Modal
        // ========================
        ...
        cmBtnDelete.addEventListener('click', async () => {
            ...
            try {
                const deleteUrl = `{{ rtrim(route('administrator.converter.log.destroy', ['id' => '__ID__']), '') }}`.replace('__ID__', id);
