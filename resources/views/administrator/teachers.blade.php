...
<head>
    ...
    <title>Kelola Guru - NusaLearn</title>
...
<body class="bg-p-xlt text-black font-sans selection:bg-p-lt selection:text-p-dark dark:bg-[#1e1b4b] dark:text-gray-100 transition-colors duration-300 relative">
    ...
    <div id="add-modal" class="modal-wrap" role="dialog" aria-modal="true">
        <div class="modal-card">
            <div class="flex items-center justify-between px-7 py-5 bg-p border-b-2 border-black dark:border-p-dark">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 bg-white border-2 border-black rounded-xl flex items-center justify-center shadow-neo-sm">
                        <i class="fa-solid fa-chalkboard-user text-p text-sm"></i>
                    </div>
                    <div>
                        <h3 class="text-lg font-black text-white tracking-tight leading-none">Tambah Guru</h3>
                        <p class="text-[11px] font-semibold text-white/70 mt-0.5 font-body">Daftarkan akun guru baru.</p>
                    </div>
                </div>
            ...
            <div class="px-7 py-6 bg-white dark:bg-[#2d2460] max-h-[80vh] overflow-y-auto no-scrollbar">
                <form action="{{ route('administrator.guru.store') }}" method="POST" class="space-y-5">
                    @csrf
                    <div>
                        <label class="block text-xs font-black text-black dark:text-white uppercase tracking-widest mb-2">Nama Lengkap</label>
                        <input name="nama" type="text" placeholder="Nama Lengkap Guru" required
        ...
        <div class="modal-card">
            <div class="flex items-center justify-between px-7 py-5 bg-p border-b-2 border-black dark:border-p-dark">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-lg font-black text-white tracking-tight leading-none">Edit Guru</h3>
                        <p class="text-[11px] font-semibold text-white/70 mt-0.5 font-body">Perbarui data tenaga pendidik.</p>
                    </div>
                </div>
            ...
            <div class="px-7 py-6 bg-white dark:bg-[#2d2460] max-h-[80vh] overflow-y-auto no-scrollbar">
                <form id="edit-form" method="POST" class="space-y-5">
                    ...
                    <div>
                        <label class="block text-xs font-black text-black dark:text-white uppercase tracking-widest mb-2">Nama Lengkap</label>
                        <input id="edit-nama" name="nama" type="text" placeholder="Nama Guru" required
                               class="w-full bg-p-xlt dark:bg-[#1e1b4b] border-2 border-black dark:border-p-dark rounded-xl py-3 px-4 text-sm font-bold text-black dark:text-white outline-none focus:border-p dark:focus:border-p-mid focus:shadow-neo-p transition-all placeholder:text-gray-400">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                        ...
                        <div>
                            <label class="block text-xs font-black text-black dark:text-white uppercase tracking-widest mb-2">Email</label>
                            <input id="edit-email" name="email" type="email" placeholder="Email" required
                                   class="w-full bg-p-xlt dark:bg-[#1e1b4b] border-2 border-black dark:border-p-dark rounded-xl py-3 px-4 text-sm font-bold text-black dark:text-white outline-none focus:border-p dark:focus:border-p-mid focus:shadow-neo-p transition-all placeholder:text-gray-400">
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-black text-black dark:text-white uppercase tracking-widest mb-2">Sekolah Asal</label>
                        <select id="edit-school" name="asal_sekolah" required
                                class="w-full bg-p-xlt dark:bg-[#1e1b4b] border-2 border-black dark:border-p-dark rounded-xl py-3 px-4 text-sm font-bold text-black dark:text-white outline-none focus:border-p dark:focus:border-p-mid focus:shadow-neo-p transition-all">
                            <option value="">Pilih Sekolah...</option>
                            @foreach($schools as $school)
                                <option value="{{ $school->nama }}">{{ $school->nama }}</option>
                    ...
                    <div class="p-4 bg-p-lt dark:bg-p-dark/30 border-2 border-dashed border-black dark:border-p-dark rounded-xl">
            ...
            <header class="h-20 border-b-2 border-black dark:border-p-dark flex items-center justify-between px-6 lg:px-10 z-20 sticky top-0 bg-white dark:bg-[#241f5c] rounded-t-2xl">
                <div class="flex items-center gap-4">
                    ...
                    <div class="hidden md:flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-2xl font-black text-gray-900 dark:text-white tracking-tight">Kelola Guru</h1>
                            <p class="text-sm text-gray-500 font-medium mt-1">Daftar tenaga pendidik yang memiliki akses ke NusaLearn.</p>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="w-full text-left border-collapse whitespace-nowrap">
                            <thead>
                                <tr class="bg-p-xlt dark:bg-p-dark/40 border-b-2 border-black dark:border-p-dark">
                                    <th class="px-6 py-4 text-xs font-black text-p-dark dark:text-purple-300 uppercase tracking-wider">Profil Guru</th>
                                    ...
                                    <th class="px-6 py-4 text-xs font-black text-p-dark dark:text-purple-300 uppercase tracking-wider text-center">Kode Akses</th>
                            ...
                            <tbody class="divide-y-2 divide-p-lt dark:divide-p-dark/30 text-sm">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-p-dark/20 transition-colors group">
                                    <td class="px-6 py-5">
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-xl bg-neo-yellow border-2 border-black dark:border-p-dark flex items-center justify-center text-black font-black text-lg shadow-neo-sm">
                                                {{ strtoupper(substr($teacher->nama ?? 'G', 0, 1)) }}
                                            </div>
                                            <div>
                                                ...
                                                <div class="font-bold text-gray-500 text-xs">
                                                    {{ '@' . $teacher->nama_pengguna }} | {{ $teacher->email }}
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-5 max-w-[200px]">
                                        <div class="text-black dark:text-white font-bold truncate flex items-center gap-2 text-sm" title="{{ $teacher->asal_sekolah }}">
                                            <i class="fa-solid fa-school text-p-mid text-xs"></i> {{ $teacher->asal_sekolah ?? '-' }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-5 text-center">
                                        @if(!empty($teacher->kode_akses))
                                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-neo-green text-black border-2 border-black text-xs font-black shadow-neo-sm" title="Terverifikasi">
                                                <div class="w-2 h-2 rounded-full bg-green-500 border border-black"></div>
                                                Aktif
                                            </span>
                                        @else
                                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-neo-red text-black border-2 border-black text-xs font-black shadow-neo-sm" title="Belum Terverifikasi">
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <span class="font-mono text-xs font-black tracking-widest bg-neo-green text-black border-2 border-black px-3 py-1.5 rounded-lg shadow-neo-sm">
                                                {{ $teacher->kode_akses ?? 'TIDAK_ADA' }}
                                            </span>
                                            <form action="{{ route('administrator.guru.generate-code', $teacher->id) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit" class="w-8 h-8 rounded-lg flex items-center justify-center bg-white border-2 border-black text-black hover:bg-neo-yellow transition-all shadow-neo-sm active:translate-x-[1px] active:translate-y-[1px] active:shadow-none" title="Generate Kode Baru">
                                    ...
                                    <td class="px-6 py-5 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <button onclick="openEditModal({{ $teacher->id }}, '{{ addslashes($teacher->nama) }}', '{{ addslashes($teacher->nama_pengguna) }}', '{{ addslashes($teacher->email) }}', '{{ addslashes($teacher->asal_sekolah) }}')"
                                            ...
                                            <form id="delete-form-{{ $teacher->id }}" action="{{ route('administrator.guru.destroy', $teacher->id) }}" method="POST" class="inline" onsubmit="event.preventDefault(); openDeleteModal('delete-form-{{ $teacher->id }}', '{{ addslashes($teacher->nama_lengkap) }}');">
                                                @csrf @method('DELETE')
                                                <button type="submit"
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-16 text-center bg-white dark:bg-[#2d2460]">
                                        ...
                                        <p class="text-gray-500 dark:text-purple-300/50 font-bold text-sm">Belum ada data guru yang didaftarkan.</p>
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
    ...
    <script>
        ...
        function openEditModal(id, nama, nama_pengguna, email, asal_sekolah) {
            const form = document.getElementById('edit-form');
            form.action = "{{ route('administrator.guru.update', 999) }}".replace('999', id);
            document.getElementById('edit-nama').value = nama;
            document.getElementById('edit-nama_pengguna').value = nama_pengguna;
            document.getElementById('edit-email').value = email;
            document.getElementById('edit-school').value = asal_sekolah;
