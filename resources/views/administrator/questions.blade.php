...
<body class="bg-p-xlt text-black font-sans selection:bg-p-lt selection:text-p-dark dark:bg-[#1e1b4b] dark:text-gray-100 transition-colors duration-300 relative">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-p-dark/20 transition-colors group">
                                    ...
                                    <td class="px-6 py-5 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <form id="delete-form-{{ $q->id }}" action="{{ route('administrator.soal.destroy', $q->id) }}" method="POST" class="inline">
                                                @csrf @method('DELETE')
            ...
            <div class="modal-scroll-body px-8 py-8 bg-white dark:bg-[#2d2460] space-y-6">
                ...
                <div id="manual-form-area" class="hidden">
                    ...
                    <form id="manual-quiz-form"
                          action="{{ route('administrator.questions.manual.store') }}"
                          method="POST"
                        ...
                        <div id="tpl-fill_blank" class="tpl-section hidden space-y-4">
                            <div class="p-4 border-2 border-dashed border-black dark:border-p-dark rounded-xl bg-neo-yellow/30">
                                ...
                                <div id="fill-sentence-list" class="space-y-3">
                                    <div class="fill-sentence-row flex gap-3">
                                        <input type="text" class="fill-sentence-dynamic w-full bg-white dark:bg-[#1e1b4b] border-2 border-black dark:border-p-dark rounded-xl px-4 py-2.5 text-sm font-bold text-black dark:text-white outline-none focus:border-p"
                                               placeholder="Tulis kalimat dengan ___ sebagai penanda blank... Contoh: Rumah adat Tolaki disebut ___"
                                               oninput="syncFillBlanks()" onchange="syncFillBlanks()">
                                        <button type="button" onclick="removeFillBlankRow(this)" class="w-10 h-10 flex items-center justify-center bg-neo-red border-2 border-black text-black rounded-xl hover:bg-red-400 flex-shrink-0" title="Hapus Kalimat">
                                            <i class="fa-solid fa-trash-can text-sm"></i>
                                        </button>
                                    </div>
                                </div>
                                <button type="button" onclick="addFillBlankRow()" class="mt-3 text-sm font-bold text-yellow-600 dark:text-yellow-400 hover:underline flex items-center gap-1">
                                    <i class="fa-solid fa-plus text-xs"></i> Tambah kalimat
                                </button>
                                
                                <input type="hidden" name="fill_sentence" id="fill-sentence-input" value="">
        ...
        <div class="modal-container modal-scroll-wrap bg-white dark:bg-[#2d2460] w-11/12 md:max-w-3xl mx-auto rounded-3xl border-2 border-black dark:border-p-dark shadow-neo-lg z-50 transform transition-all scale-95 opacity-0">
            ...
            <div class="modal-scroll-body">
                ...
                <div id="ai-step-2" class="hidden px-8 py-8 bg-white dark:bg-[#2d2460] space-y-6">
                    ...
                    <form action="{{ route('administrator.soal.store') }}" method="POST" id="confirm-save-form" class="space-y-4">
                        @csrf
            ...
            <div class="modal-scroll-body px-8 py-8 bg-white dark:bg-[#2d2460] space-y-6">
                <form id="edit-quiz-form" action="" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div id="edit-tpl-fill_blank" class="edit-tpl-section hidden space-y-4">
                        <div class="p-4 border-2 border-dashed border-black dark:border-p-dark rounded-xl bg-neo-yellow/30">
                            <p class="text-[11px] text-gray-500 dark:text-gray-400 mb-3">Gunakan <kode class="bg-white px-1 py-0.5 rounded border border-gray-200">___</kode> sebagai penanda kosong.</p>
                            <div id="edit-fill-sentence-list" class="space-y-3">
                                <!-- Dynamic rows go here via JS -->
                            </div>
                            <button type="button" onclick="addEditFillBlankRow()" class="mt-3 text-sm font-bold text-yellow-600 hover:underline flex items-center gap-1">
                                <i class="fa-solid fa-plus text-xs"></i> Tambah kalimat
                            </button>
                            <input type="hidden" name="fill_sentence" id="edit-fill-sentence-input" value="">
    ...
    function syncFillBlanks() {
        const inputs = document.querySelectorAll('.fill-sentence-dynamic');
        const sentences = Array.from(inputs).map(inp => inp.value).filter(val => val.trim() !== '');
        document.getElementById('fill-sentence-input').value = sentences.join('\n');
        parseFillBlanks();
    }

    function addFillBlankRow() {
        const list = document.getElementById('fill-sentence-list');
        const row = document.createElement('div');
        row.className = 'fill-sentence-row flex gap-3';
        row.innerHTML = `
            <input type="text" class="fill-sentence-dynamic w-full bg-white dark:bg-[#1e1b4b] border-2 border-black dark:border-p-dark rounded-xl px-4 py-2.5 text-sm font-bold text-black dark:text-white outline-none focus:border-p"
                   placeholder="Kalimat soal selanjutnya... (Gunakan ___ untuk blank)"
                   oninput="syncFillBlanks()" onchange="syncFillBlanks()">
            <button type="button" onclick="removeFillBlankRow(this)" class="w-10 h-10 flex items-center justify-center bg-neo-red border-2 border-black text-black rounded-xl hover:bg-red-400 flex-shrink-0" title="Hapus Kalimat">
                <i class="fa-solid fa-trash-can text-sm"></i>
            </button>
    ...
    function removeFillBlankRow(btn) {
        btn.closest('.fill-sentence-row').remove();
        syncFillBlanks();
    }
    ...
    function parseFillBlanks() {
        ...
        // Simpan nilai lama
        const oldVals = Array.from(existing).map(el => el.querySelector('input').value);
    ...
    async function triggerGenerate() {
        ...
        try {
            const res = await fetch('{{ route("administrator.questions.generate") }}', {
                method: 'POST',
                headers: {
    ...
    async function generateManualExplanation() {
        ...
        try {
            const res = await fetch('{{ route("administrator.questions.generate-explanation") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
    ...
    function editQuestion(id, templateType, data, materialId, difficultyWeight, kelasId) {
        ...
        form.action = `{{ url('admin/administrator/soal') }}/${id}`;
        ...
        if (templateType === 'multiple_choice') {
            ...
            const rawSentences = (data.teks_soal || data.question_text || '').split('\n');
            const list = document.getElementById('edit-fill-sentence-list');
            list.innerHTML = '';
            
            // Tambahkan baris sesuai jumlah kalimat, atau minimal 1 baris
            if (rawSentences.length > 0) {
                rawSentences.forEach(s => {
                    if (s.trim()) addEditFillBlankRow(s.trim());
                });
            }
    ...
    function syncEditFillBlanks() {
        const inputs = document.querySelectorAll('.edit-fill-sentence-dynamic');
        const sentences = Array.from(inputs).map(inp => inp.value).filter(val => val.trim() !== '');
        document.getElementById('edit-fill-sentence-input').value = sentences.join('\n');
        
        const currentAnswerInputs = document.querySelectorAll('#edit-fill-blanks-answers input');
        const currentAnswers = Array.from(currentAnswerInputs).map(inp => inp.value);
        parseEditFillBlanks(currentAnswers);
    }

    function addEditFillBlankRow(val = '') {
        const list = document.getElementById('edit-fill-sentence-list');
        const row = document.createElement('div');
        row.className = 'edit-fill-sentence-row flex gap-3';
        row.innerHTML = `
            <input type="text" class="edit-fill-sentence-dynamic w-full bg-white border-2 border-black rounded-xl px-4 py-2 text-sm font-bold outline-none"
                   placeholder="Kalimat soal... (Gunakan ___ untuk blank)"
                   value="${val.replace(/"/g, '&quot;')}"
                   oninput="syncEditFillBlanks()" onchange="syncEditFillBlanks()">
            <button type="button" onclick="removeEditFillBlankRow(this)" class="w-10 h-10 flex items-center justify-center bg-neo-red border-2 border-black text-black rounded-xl hover:bg-red-400 flex-shrink-0">
                <i class="fa-solid fa-trash-can text-sm"></i>
            </button>
        `;
        list.appendChild(row);
    ...
    function removeEditFillBlankRow(btn) {
        btn.closest('.edit-fill-sentence-row').remove();
        syncEditFillBlanks();
    }
