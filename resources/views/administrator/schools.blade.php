...
<head>
    ...
    <title>Kelola Sekolah - NusaLearn</title>
    ...
    <style>
        ...
        .modal-card { background:#fff; border:3px solid #000; border-radius:24px; box-shadow:8px 8px 0 #000; width:90%; max-width:500px; overflow:hidden; animation:em-pop 0.3s cubic-bezier(0.34,1.56,0.64,1) both; }
...
<body class="bg-p-xlt text-black font-sans selection:bg-p-lt selection:text-p-dark dark:bg-[#1e1b4b] dark:text-gray-100 transition-colors duration-300 relative">
    ...
    <div id="add-modal" class="modal-wrap" role="dialog" aria-modal="true">
        <div class="modal-card">
            <div class="flex items-center justify-between px-7 py-5 bg-p border-b-2 border-black dark:border-p-dark">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 bg-white border-2 border-black rounded-xl flex items-center justify-center shadow-neo-sm">
                        <i class="fa-solid fa-school text-p text-sm"></i>
                    </div>
                    <div>
                        <h3 class="text-lg font-black text-white tracking-tight leading-none">Tambah Sekolah</h3>
                        <p class="text-[11px] font-semibold text-white/70 mt-0.5 font-body">Daftarkan sekolah baru ke sistem.</p>
                    </div>
                </div>
            ...
            <div class="px-7 py-6 bg-white dark:bg-[#2d2460]">
                <form action="{{ route('administrator.sekolah.store') }}" method="POST" class="space-y-5">
                    @csrf
                    <div>
                        <label class="block text-xs font-black text-black dark:text-white uppercase tracking-widest mb-2">Nama Sekolah</label>
                        <input name="nama" type="text" placeholder="Contoh: SMAN 1 Kendari"
                               class="w-full bg-p-xlt dark:bg-[#1e1b4b] border-2 border-black dark:border-p-dark rounded-xl py-3 px-4 text-sm font-bold text-black dark:text-white outline-none focus:border-p dark:focus:border-p-mid focus:shadow-neo-p transition-all placeholder:text-gray-400" required>
                    </div>
                    <div>
                        <label class="block text-xs font-black text-black dark:text-white uppercase tracking-widest mb-2">Alamat Sekolah</label>
                        <textarea name="alamat" placeholder="Alamat lengkap sekolah" rows="3"
                                  class="w-full bg-p-xlt dark:bg-[#1e1b4b] border-2 border-black dark:border-p-dark rounded-xl py-3 px-4 text-sm font-bold text-black dark:text-white outline-none focus:border-p dark:focus:border-p-mid focus:shadow-neo-p transition-all placeholder:text-gray-400"></textarea>
                    </div>
                    <div>
                        <label class="block text-xs font-black text-black dark:text-white uppercase tracking-widest mb-2">Kode Pos</label>
                        <input name="kode_pos" type="text" placeholder="Contoh: 93111"
                               class="w-full bg-p-xlt dark:bg-[#1e1b4b] border-2 border-black dark:border-p-dark rounded-xl py-3 px-4 text-sm font-bold text-black dark:text-white outline-none focus:border-p dark:focus:border-p-mid focus:shadow-neo-p transition-all placeholder:text-gray-400">
                    </div>
        ...
        <div class="modal-card">
            <div class="flex items-center justify-between px-7 py-5 bg-p border-b-2 border-black dark:border-p-dark">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-lg font-black text-white tracking-tight leading-none">Edit Sekolah</h3>
                        <p class="text-[11px] font-semibold text-white/70 mt-0.5 font-body">Perbarui data sekolah terdaftar.</p>
                    </div>
                </div>
            ...
            <div class="px-7 py-6 bg-white dark:bg-[#2d2460]">
                <form id="edit-form" method="POST" class="space-y-5">
                    ...
                    <div>
                        <label class="block text-xs font-black text-black dark:text-white uppercase tracking-widest mb-2">Nama Sekolah</label>
                        <input id="edit-nama" name="nama" type="text" placeholder="Nama Sekolah"
                               class="w-full bg-p-xlt dark:bg-[#1e1b4b] border-2 border-black dark:border-p-dark rounded-xl py-3 px-4 text-sm font-bold text-black dark:text-white outline-none focus:border-p dark:focus:border-p-mid focus:shadow-neo-p transition-all placeholder:text-gray-400" required>
                    </div>
                    <div>
                        <label class="block text-xs font-black text-black dark:text-white uppercase tracking-widest mb-2">Alamat Sekolah</label>
                        <textarea id="edit-alamat" name="alamat" placeholder="Alamat lengkap sekolah" rows="3"
                                  class="w-full bg-p-xlt dark:bg-[#1e1b4b] border-2 border-black dark:border-p-dark rounded-xl py-3 px-4 text-sm font-bold text-black dark:text-white outline-none focus:border-p dark:focus:border-p-mid focus:shadow-neo-p transition-all placeholder:text-gray-400"></textarea>
                    </div>
                    <div>
                        <label class="block text-xs font-black text-black dark:text-white uppercase tracking-widest mb-2">Kode Pos</label>
                        <input id="edit-postal-kode" name="kode_pos" type="text" placeholder="Kode Pos"
                               class="w-full bg-p-xlt dark:bg-[#1e1b4b] border-2 border-black dark:border-p-dark rounded-xl py-3 px-4 text-sm font-bold text-black dark:text-white outline-none focus:border-p dark:focus:border-p-mid focus:shadow-neo-p transition-all placeholder:text-gray-400">
                    </div>
                    <div class="flex gap-3 pt-2 border-t-2 border-p-lt dark:border-p-dark">
            ...
            <header class="h-20 border-b-2 border-black dark:border-p-dark flex items-center justify-between px-6 lg:px-10 z-20 sticky top-0 bg-white dark:bg-[#241f5c] rounded-t-2xl">
                <div class="flex items-center gap-4">
                    ...
                    <div class="hidden md:flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-2xl font-black text-gray-900 dark:text-white tracking-tight">Kelola Sekolah</h1>
                            <p class="text-sm text-gray-500 font-medium mt-1">Daftar sekolah yang berpartisipasi dalam NusaLearn.</p>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="w-full text-left border-collapse whitespace-nowrap">
                            <thead>
                                <tr class="bg-p-xlt dark:bg-p-dark/40 border-b-2 border-black dark:border-p-dark">
                                    <th class="px-6 py-4 text-xs font-black text-p-dark dark:text-purple-300 uppercase tracking-wider">Nama Sekolah</th>
                                    <th class="px-6 py-4 text-xs font-black text-p-dark dark:text-purple-300 uppercase tracking-wider">Alamat</th>
                                    <th class="px-6 py-4 text-xs font-black text-p-dark dark:text-purple-300 uppercase tracking-wider text-center">Kode Pos</th>
                            ...
                            <tbody class="divide-y-2 divide-p-lt dark:divide-p-dark/30 text-sm">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-p-dark/20 transition-colors group">
                                    <td class="px-6 py-5">
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-xl bg-neo-cyan border-2 border-black dark:border-p-dark flex items-center justify-center text-black font-black text-lg shadow-neo-sm">
                                                {{ strtoupper(substr($school->nama, 0, 1)) }}
                                            </div>
                                            <div>
                                    ...
                                    <td class="px-6 py-5 max-w-[300px]">
                                        <div class="text-black dark:text-white font-bold truncate text-sm" title="{{ $school->alamat }}">
                                            {{ $school->alamat ?? 'Tidak ada alamat' }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-5 text-center">
                                        <span class="font-mono text-xs font-black tracking-widest bg-neo-yellow text-black border-2 border-black px-3 py-1 rounded-lg shadow-neo-sm">
                                            {{ $school->kode_pos ?? '-' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-5 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <button onclick="openEditModal({{ $school->id }}, '{{ addslashes($school->nama) }}', '{{ addslashes($school->alamat) }}', '{{ addslashes($school->kode_pos) }}')"
                                            ...
                                            <form id="delete-form-{{ $school->id }}" action="{{ route('administrator.sekolah.destroy', $school->id) }}" method="POST" class="inline" onsubmit="event.preventDefault(); openDeleteModal('delete-form-{{ $school->id }}', '{{ addslashes($school->nama) }}');">
                                                @csrf @method('DELETE')
                                                <button type="submit"
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-16 text-center bg-white dark:bg-[#2d2460]">
                                        ...
                                        <p class="text-gray-500 dark:text-purple-300/50 font-bold text-sm">Belum ada data sekolah yang didaftarkan.</p>
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
    ...
    <script>
        ...
        function openEditModal(id, nama, alamat, kode_pos) {
            const form = document.getElementById('edit-form');
            form.action = "{{ route('administrator.sekolah.update', 999) }}".replace('999', id);
            document.getElementById('edit-nama').value = nama;
            document.getElementById('edit-alamat').value = alamat;
            document.getElementById('edit-postal-kode').value = kode_pos;
